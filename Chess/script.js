...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let userColor = 'w';
    var bet = prompt("Введіть ставку (в доларах): (ставку можна збільшити в ході гри за згодою супернка)") // Initialize the user's color as white
    ...
    document.querySelector('.set-pos').addEventListener('click', () => {
        const fen = prompt("Введіть нову ставку: (лише за згодою суперника)");
        if (fen !== null) {
            if (game.load(fen)) {
                board.position(fen);
                moveHistory.textContent = '';
                ...
                alert("Суперних відхилив пропозицію підвищити ставку.");
            }
        }
    });
